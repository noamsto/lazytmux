...
| `tmux-agent-usage-claude` / `-codex` / `-cursor` | called by the dispatcher | Provider impls: curl the vendor usage endpoint with the CLI's own stored token (`~/.claude/.credentials.json`, `~/.codex/auth.json`, `~/.config/cursor/auth.json` — no extra API keys) → normalized `{windows:[{label,pct}], monthly:{label,pct}}`. Failed fetches keep the previous cache. Cursor's endpoint only caps monthly requests, so enterprise tiers (all-null caps) write an empty cache and stay hidden. |
...
| `lztmux-remote-open` | `prefix + s` remote section / CLI | Opens a remote tmux session as native local windows via the control-mode bridge (`lztmux-remote-bridge-daemon`). Outbound SSH only. |
...
- **Uncapped agents drop out.** A provider with nothing to report (Cursor on
  enterprise: every cap null) writes an empty cache and renders nothing.
...
- **`@window_bridge_name`** — daemon-owned remote window name for a `@bridge_win` mirror window; read only by reflow's bridge branch (#196).
