...
- **Window options are the source of truth.** `tmux-issue-stamp` (backgrounded
  ...
  (driven by the `@lztmux-pr-tick` monitor hook, not status-format[0] — a
...
- **Cache files are the source of truth.** `tmux-agent-usage` (driven by the
  `@lztmux-usage-tick` monitor hook, not status-format[0] — a control-mode
  ...
  normalize vendor API responses into `/tmp/lazytmux-agent-usage/<agent>.json`
