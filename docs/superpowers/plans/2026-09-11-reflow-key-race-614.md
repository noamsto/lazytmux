...
- [x] **4. D2: bounded foreground, detached waiter** (implement: escalated).
  - **Bounded foreground.** Keep HEAD's 40 × 0.05s budget, so synchronous
    callers stay capped at ~2s. Those callers are `session-window-changed`,
...
- [x] **5. Tests** in `tests/reflow-fanout.bats`. `setup()` gives the
  ...
  re-exec `$0`.
  - [x] **Foreground budget.** Hold the lock and run `timeout 5 … --force`.
    Assert exit 0 and `sentinel`. Release the lock, then poll up to 5s for
    `1:200:0`.
  - [x] **Dead holder.** Set `OG_LOCK_STALE_SECONDS=5` and never release the
    lock dir. Assert exit 0 and `sentinel`, then poll up to 12s for
    `1:200:0`. Staleness is whole-second, so 3 could let the foreground steal
...
- [x] **8. Spec.** Match the code: the waiter exits 0 at its deadline, AC2 uses
  `OG_LOCK_STALE_SECONDS=5`, and the D1 test polls. Record the final e2e table.
...
- [ ] **10. Commit.** From inside `nix develop`/direnv, so the pre-commit hooks
...
- [ ] **11. Rebase onto `origin/main`**, then run the gate on the rebased tree:
  `nix build .#default`, `nix build .#lint`, `nix flake check`.
- [ ] **12. `/deslop`.** If it changes anything, re-run the gate.
- [ ] **13. Push and open the PR** with `gh pr create --assignee @me`. The body
  contains:
  - `Closes #614`
