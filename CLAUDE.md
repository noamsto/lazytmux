...
| `tmux-pr-enrich` | `@og-pr-tick` monitor hook (every 5s, `--tick`); `prefix + i` `r` (`--force`) | Background PR poller. Every gh call runs inside a checkout of the branch's repo (`--dir` / window `@worktree`/`@git_root`) — the poller's own cwd is the tmux server's, not a repo. Full pass groups windows by repo: fast PR-identity batches run every `prRefreshSeconds`, while check-rollup batches run every `prCheckRefreshSeconds`; per-branch fallback only handles heads with no open PR (cached at `/tmp/og-pr/`, 60s TTL, 1h for merged/closed, lock-guarded via the portable `acquire_lock`). Writes `@pr_number`/`@pr_title`/`@pr_state`/`@pr_check_state`/`@pr_url`/`@pr_draft`/`@pr_review`/`@pr_auto_merge`/`@pr_check_progress`. A repo whose last applied rollup was pending keeps a `<sha1>.checks-pending` marker (content: the epoch it was first seen pending; mtime: its last checks refresh start) and re-polls checks about every 30s through a `--tick-run-pending` checks-only pass that skips the identity batch — for at most 10 minutes per repo, then back to `prCheckRefreshSeconds` until the checks settle. A pass removes markers for repos that no longer have a window, and `prefix + i` `r` on a pending PR arms the fast cadence. A push that sends a settled PR back to pending waits for the slow check refresh (the identity batch carries no head SHA; `headRefOid` is the follow-up). Single-target mode exits immediately on a `@bridge_win` window — a mirror's branch belongs to the launcher's repo, not the remote content — so a bridged `[r]` is routed to the remote's own copy by the `enrich-refresh` ctl verb instead (#598). |
