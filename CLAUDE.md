...
- **tmux `-F` formats are `|`-delimited, never tab-delimited.** tmux rewrites non-printable bytes in command output to `_` unless the *querying client's* locale is UTF-8 (the client decides, not the server), so a tab-delimited format collapses to a single field and every consumer parses garbage. That silently deleted every agent state file once per 5s and made `nix flake check` flaky on both platforms (#373); the same defect still spawns a duplicate window per `wt switch` from `modules/home-manager.nix` (#378). `checks.<system>.tmux-format-delimiter-assertions` enforces this over `scripts/` — it rejects both a raw control byte and a `$'…\t…'` escape, and scans whole files so a format hidden behind `-F "$FMT"` is caught at its definition. `\n` is rejected too — tmux mangles a newline exactly like a tab, which had collapsed reflow's own 3-line `-p` read. Multi-byte UTF-8 is allowed (reflow's status-format strings carry `├─` glyphs, which are printable). `modules/home-manager.nix` and `picker/**` still carry tab-delimited formats — tracked separately, which is why the check's scope is `scripts/`.
