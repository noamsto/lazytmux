...
in {
  options.programs.lazytmux = {
    ...
    carouselDiagramTools = lib.mkOption {
      type = lib.types.listOf lib.types.package;
      default = lib.optional (carousel-default != null) carousel-default ++ [pkgs.resvg];
      defaultText = lib.literalExpression "lib.optional (carousel-default != null) carousel-default ++ [pkgs.resvg]";
      description = ''
        ...
        PNG images for the carousel. The hook runs `aeye render-diagram`, which
        embeds d2 (so `d2` is no longer needed on PATH) and shells out only to
        `resvg` — so it resolves `aeye` and `resvg` from PATH (the same
        PATH-reach reason as popupTools). Only installed when the agent-carousel
        flake input is wired in (carousel-toggle != null).
