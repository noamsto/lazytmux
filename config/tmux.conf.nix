{
  ...
  mkScriptWithDeps = name: let
    ...
    patched = builtins.replaceStrings ["claude-status "] ["${claude-status-bin} "] raw;
  in
  ...
  # Scripts that need claude-status substitution
  pickerScripts = ["tmux-session-picker" "tmux-window-picker"];

  # Individual script references for full store paths in config
  script = lib.genAttrs scriptNames (name:
    if builtins.elem name pickerScripts
