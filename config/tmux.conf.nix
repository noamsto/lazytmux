{
  ...
  tmuxConfText = pkgs.writeText "tmux.conf" ''
    ...
    set -g status-format[0] "#(${script.tmux-update-icons}/bin/tmux-update-icons '#{session_name}')${lib.optionalString enrichEnable "#(${script.tmux-pr-enrich}/bin/tmux-pr-enrich --tick)"}#[align=left,bg=#{@thm_bg}]#{?client_prefix,#[fg=#{@thm_red}#,bold],#[fg=#{@thm_mauve}]} #{@icon_session} #S  #{?#{@issue_id},#[fg=#{@thm_blue}#,bold]#{?#{==:#{@issue_provider},linear},${enrichIconSet.linear},${enrichIconSet.github}} #{@issue_id} #[fg=#{@thm_text}#,nobold]#{@issue_title},#[fg=#{@thm_blue}#,bold]#{@icon_branch} #(${script.tmux-branch-display}/bin/tmux-branch-display '#{@branch}' '#{pane_current_path}')}#{?#{&&:#{@pr_number},#{!=:#{@pr_number},none}},  #{?#{==:#{@pr_check_state},failure},#[fg=#{@thm_red}],#{?#{==:#{@pr_check_state},pending},#[fg=#{@thm_peach}],#{?#{==:#{@pr_state},merged},#[fg=#{@thm_mauve}],#{?#{==:#{@pr_state},closed},#[fg=#{@thm_overlay_0}],#[fg=#{@thm_green}]}}}}#{?#{==:#{@pr_check_state},failure},${enrichIconSet.failure},#{?#{==:#{@pr_check_state},pending},${enrichIconSet.pending},#{?#{==:#{@pr_state},merged},${enrichIconSet.merged},${enrichIconSet.success}}}} ###{@pr_number} #{@pr_title},}  #[fg=#{@thm_subtext_0},nobold]#{@icon_dir} #(${script.tmux-dir-display}/bin/tmux-dir-display '#{@branch}' '#{pane_current_path}' '#{@git_root}')  #[fg=#{@thm_overlay_1}]#(${script.claude-status}/bin/claude-status --session '#{session_name}' --format icon-color) #[align=right,fg=#{@thm_subtext_0}]#{@active_pane_icon} #{pane_current_command} "

    # Lines 1-3: Window list (dynamically generated by tmux-reflow-windows hook)
    set -g status-format[1] "#[align=left,bg=#{@thm_bg}]#[fg=#{@thm_overlay_1}] ╰─ #{W:#[range=window|#{window_index}]#{?window_active,#[fg=#{@thm_green}#,bold]#{window_index}: #{?#{==:#{@labels_mode},long},#{@window_label_long},#{@window_label_short}} #{@window_icon_display},#[fg=#{@thm_subtext_0}#,nobold]#{window_index}: #{?#{&&:#{@pr_number},#{!=:#{@pr_number},none}},#{?#{==:#{@pr_check_state},failure},#[fg=#{@thm_red}],#{?#{==:#{@pr_check_state},pending},#[fg=#{@thm_peach}],#{?#{==:#{@pr_state},merged},#[fg=#{@thm_mauve}],#[fg=#{@thm_fg}]}}},#[fg=#{@thm_fg}]}#{?#{==:#{@labels_mode},long},#{@window_label_long},#{@window_label_short}} #{@window_icon_display}}#{?window_zoomed_flag, 󰁌,}#[norange]#{?window_end_flag,, #[fg=#{@thm_subtext_0}#,nobold]│ }}"
