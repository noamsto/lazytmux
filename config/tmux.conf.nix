{
  ...
  tmuxConfText = pkgs.writeText "tmux.conf" ''
    ...
    set -g status-format[1] "#[align=left,bg=#{@thm_bg}]#[fg=#{@thm_overlay_1}] ╰─ #{W:#[range=window|#{window_index}]#[nobold]#{?window_active,#[fg=#{@thm_lavender}],#[fg=#{@thm_subtext_0}]}#{window_index}: #[bold]#{@window_label_id}#[nobold]#{?#{==:#{@labels_mode},long},#{@window_label_rest_long},#{@window_label_rest_short}} #{@window_icon_display}#{?window_zoomed_flag, 󰁌,}#{?#{&&:#{@pr_number},#{!=:#{@pr_number},none}},#{?#{==:#{@pr_check_state},failure},#[fg=#{@thm_red}],#{?#{==:#{@pr_check_state},pending},#[fg=#{@thm_peach}],#{?#{==:#{@pr_state},merged},#[fg=#{@thm_mauve}],#[fg=#{@thm_green}]}}},}#{@window_pr_plain}#[norange]#{?window_end_flag,, #[fg=#{@thm_subtext_0}#,nobold]│ }}"
