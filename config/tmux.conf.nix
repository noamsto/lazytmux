{
  ...
  tmuxConfText = pkgs.writeText "tmux.conf" ''
    ...
    bind s display-popup -E -b rounded -w 70% -h 50% -T ' Sessions ' -s 'bg=#{@thm_mantle}' -S 'fg=#{@thm_overlay_1},bg=#{@thm_mantle}' '${script.tmux-session-picker}/bin/tmux-session-picker'
