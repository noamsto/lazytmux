...
4. **`^o`, not `ctrl+r`** (DECOMPOSITION §4; spec D10 already says `^o`, so this
   settles the decomposition against the spec rather than deviating from it).
   Neither key is bound anywhere in `picker/` — checked across `handleKey`,
   `handleWallKey`, `handleFocusedKey` and `handleWallQueryKey` — so there is no
   collision either way, and `prefix + R` is a tmux-level bind that never reaches
   the picker's keymap. `^o` wins on reading as "open" where `^r` reads as
   refresh/reverse-search.
...
## Resume state (`9366a01`)

The first worker on this issue died mid-implementation and its tree was preserved
as `9366a01`; that commit implemented ahead of ticking boxes, so the checkboxes
below were rewritten from the code rather than trusted. As resumed: the launcher,
the wrapper script, and the nix wiring existed; **no `picker/*.go` file had been
touched**, and `flake.nix` already referenced a `tests/remote-picker.bats` that
did not exist — so `nix flake check` was red on arrival. Steps 11a–14 describe
behaviour the script already had but whose bats coverage was still owed; they are
ticked once that coverage lands, not on the script's existence.
...
- [x] **Step 1: red test for the empty-`win` blank mirror.** Add a case to
...
- [x] **Step 2: make an empty `win` fatal.** Guard the `:137-142` result. Keep the
...
- [x] **Step 3: red tests for `LZTMUX_REMOTE_NEW_DIR`.** Cases: (a) session absent
...
- [x] **Step 4: implement the `LZTMUX_REMOTE_NEW_DIR` branch.** Structurally mirror
...
- [x] **Step 5: emit payload builder, test-first.** The picker only ever *writes*
...
- [x] **Step 6: emit-mode target resolution, test-first.** Pure function: a
...
- [x] **Step 7: wire the mode into the model.** `--remote-pick` (bare) through
...
- [x] **Step 8: the emit branch in `activateCurrent`** (`:1077-1099`), writing
...
- [x] **Step 9: the both-empty guard.** With no remote server *and* no `zoxide`,
...
- [x] **Step 10: model-level tests.** `--remote-pick` builds no Remote section;
...
- [x] **Step 11a: the bash `key=value` parser + leg-1 value validation,
...
- [x] **Step 11b: the remote role (`--probe`, `--serve <token>`), test-first.**
...
- [x] **Step 12: the local role's three legs, test-first with `ssh` stubbed.** Leg
...
- [x] **Step 13: cancel vs. choice vs. failure.** The file is pre-created, so
...
- [x] **Step 14: handoff, test-first.** This is the local half of AC2, so assert
...
- [x] **Step 15: `shellcheck` both roles.** Required by house rules; `shfmt` uses
  tabs here.
...
- [x] **Step 16: gate parse + `new-pane` argv, test-first.** Pure functions. The
...
- [x] **Step 17: the `^o` case.** Active only when `item.remoteHost != ""`, so it
...
- [x] **Step 18: the hint line — both changes, since this step owns
  `render_list.go`.**
...
- [x] **Step 19: build the script.** `scriptNames` += `lztmux-remote-picker`, plus
...
- [x] **Step 20: `set -g @remote_pick_bin '<store path>'`.** An option repoints on
  a config reload alone; the tmux server's PATH is frozen until restart (#336).

- [x] **Step 21: install on the remote's PATH.** `home.packages` +=
...
- [x] **Step 22: `flake.nix`** — the launcher cases go in
...
- [x] **Step 23: `CLAUDE.md`.** Update the `tmux-session-picker` row (`^o`) and the
...
- [x] **Step 24: `README.md`** "Remote tmux bridge": the `^o` escape hatch, the
...
- [x] **Step 25: commit the plan + spec** under `docs/superpowers/`, and
