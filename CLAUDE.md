...
| `tmux-splash-maybe` | `client-attached[50]` / `client-session-changed[50]` hooks (backgrounded) | Eligibility gate for the welcome buffer: shows the splash once per tmux server (global `@splash_shown`) on a fresh 1-window/1-pane session whose pane is at a shell; opens `tmux-splash` in a fullscreen popup. When the attaching client came in over ssh (detected via `SSH_CONNECTION` in the session's environment table), honors `programs.lazytmux.splash.remote` (`full`/`static`/`skip`) instead of always launching the full animated splash. |
