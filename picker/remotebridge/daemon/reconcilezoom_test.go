...
// zoomed tiled pane (measured on next-3.8); any tiled -Z toggles window zoom
// and leaves the float intact. Assert the toggle lands on a tiled local pane.
func TestReconcileZoomToggleNeverTargetsAFloat(t *testing.T) {
	...
	if len(zoomTargets) == 0 {
		t.Fatal("no resize-pane -Z issued; want a tiled-pane zoom toggle")
	}
	for _, target := range zoomTargets {
		if target == "%l9" {
			t.Errorf("resize-pane -Z targeted float local id %q; want a tiled pane", target)
		}
		if target != "%l0" {
			t.Errorf("resize-pane -Z target = %q, want %%l0 (tiled fallback when remoteActive is a float)", target)
		}
	}
